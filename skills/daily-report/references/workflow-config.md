...
```json
{
  "workflow": {
    ...
    "project_scope": {
      "mode": "all",
      "project_roots": [],
      "skip_without_today_conversation": true
    }
  }
}
```
...
- `project_scope.skip_without_today_conversation` 固定为 `true`：项目在本次统计目标工作日没有对话记录时，跳过该项目的今日工作和明日计划输入；不使用任务记录、Git、其他日期或最近对话补齐，也不删除日报已有历史内容。
...
目标日报日期与项目记录的统计范围必须分别计算：相对日期按 `Asia/Shanghai` 解析，“截至昨天的本周总结”以昨天为目标日报日期、以本周一至昨天为统计范围。默认读取当前工具的项目对话记录和任务记录；项目没有本次统计目标工作日的对话时直接跳过，不生成空项目块或明日计划，不回退到其他日期或 Git。只有用户明确要求时才读取 Git。统计范围和数据来源只出现在执行回执中，不写入日报正文或标题。
