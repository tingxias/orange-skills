...
当用户要求根据当前工具中的项目和任务生成日报时，先按“使用前确认”取得 Producer Key 或读取已保存 Producer 配置的明确授权，再询问：“使用默认范围（本周全部项目），还是指定项目？”未得到范围选择前不读取和整理项目任务。

“本周”按运行端本地时区计算，只包含本周一 `00:00` 到当前时间。用户保持默认时，读取当前工具可见的全部项目及其本周任务；用户选择指定项目时，继续取得项目名称或路径，只读取匹配项目。当前工具不能提供项目或任务列表时，说明缺失来源并请用户补充，不猜测工作内容。

将任务按项目和状态归并为简明的结果性总结。同一项目、同一状态的多个任务合并成一条，只保留关键结果、当前进展、明确风险和已经确定的下一步；不逐条复制任务标题、聊天记录或操作步骤。同一事项从多个来源出现时只保留一份，没有可靠状态或时间的信息不自行归类。
...
保持默认范围时使用 `scope: {"mode":"all","projectRoots":[]}`；指定项目时使用 `scope.mode=whitelist`，并把确认后的项目名称或路径写入 `projectRoots`。
